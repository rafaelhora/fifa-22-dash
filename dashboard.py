...
with container_intro:
    ...
    st.warning('Caution, showing the unfiltered raw dataset can take minutes.')
...
def most_promising(wage_range, value_range, age_range, current_rating_range, potential_range,player_position, top_n):
    ...
    promising_players = promising_players[promising_players['Wage'].between(wage_range[0], wage_range[1]) & promising_players['Value'].between(value_range[0], value_range[1])  & promising_players['Age'].between(age_range[0], age_range[1]) & promising_players['Best Overall Rating'].between(current_rating_range[0], current_rating_range[1]) & promising_players['Potential'].between(potential_range[0], potential_range[1])]
...
with col1:
    wage_range = st.slider("Wage Range", min(df.Wage), max(df.Wage),(min(df.Wage), max(df.Wage)), step = 500.0)
    value_range = st.slider("Value Range", min(df.Value), max(df.Value),(min(df.Value), max(df.Value)), step = 50000.0)
    age_range = st.slider("Age Range", min(df.Age), max(df.Age),(min(df.Age), max(df.Age)))
with col2:
    current_rating_range = st.slider("Current rating", min(df['Best Overall Rating']), max(df['Best Overall Rating']), (min(df['Best Overall Rating']), max(df['Best Overall Rating'])))
...
st.dataframe(most_promising(wage_range, value_range, age_range, current_rating_range, potential_range,player_position, top_n))
